...
export const sortList: SortItem[] = [
  ...
  { name: 'alphabetical order (DESC)', sortProperty: SortPropertyEnum.TITLE_DESC },
  { name: 'alphabetical order (ASC)', sortProperty: SortPropertyEnum.TITLE_ASC },
];
...
export const Sort: React.FC<SortPopupProps> = React.memo(({ value }) => {
  ...
  React.useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (_event.path && _event.target && sortRef.current && !_event.path.includes(sortRef.current)) {
        setOpen(false);
      }
    ...
    <div ref={sortRef} className="sort">
      <div className="sort__label">
        <svg
          ...
          <path
            d="M10 5C10 5.16927 9.93815 5.31576 9.81445 5.43945C9.69075 5.56315 
